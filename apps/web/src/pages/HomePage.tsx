...
// HomePage composes the landing hero, the analysis form, and the result panel.
function HomePage() {
        ...
        <section className="grid flex-1 items-center gap-10 py-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-14">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">
              Paste any article or news text and let AI analyze its credibility.
              Get a clean prediction, confidence score, and a visual verdict in one focused workflow.
            </p>

            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <p className="text-sm font-medium text-white">Paste article</p>
                <p className="mt-1 text-sm leading-6 text-slate-400">
                  Add the news text you want to verify.
                </p>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <p className="text-sm font-medium text-white">Analyze</p>
                <p className="mt-1 text-sm leading-6 text-slate-400">
                  Call the FastAPI model backend instantly.
                </p>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <p className="text-sm font-medium text-white">Review result</p>
                <p className="mt-1 text-sm leading-6 text-slate-400">
                  See Fake or Real plus the confidence score.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            {result ? (
              ...
              <div className="rounded-xl border border-dashed border-white/15 bg-white/5 p-6 text-sm leading-6 text-slate-400">
                Prediction result will appear here after analysis.
              </div>
            )}
          </div>
        </section>
